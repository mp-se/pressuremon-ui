...
describe('PushSettingsView', () => {
  ...
  it('renders wifi direct settings fields', () => {
    const wrapper = mountView()
    expect(wrapper.text()).toContain('Direct SSID')
    expect(wrapper.text()).toContain('Direct Password')
    expect(wrapper.text()).toContain('Use wifi direct in measurement mode')
  })
  ...
  it('updates all push settings models from child component emits', async () => {
    ...
    expect(mocks.config.wifi_direct_ssid).toBe('Direct SSID updated')
    expect(mocks.config.wifi_direct_pass).toBe('Direct Password updated')
    expect(mocks.config.use_wifi_direct).toBe(true)
  })
